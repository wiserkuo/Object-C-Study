...
- (void)beginScrollWithDelay:(BOOL)delay {
    switch (self.marqueeType) {
        case MLContinuousOneTimes:
            [self scrollContinuousOneTimeWithInterval:[self durationForInterval:self.animationDuration] after:(delay ? self.animationDelay : 0.0)];
            break;
        case MLContinuous:
        case MLContinuousReverse:
...
- (void)scrollContinuousOneTimeWithInterval:(NSTimeInterval)interval after:(NSTimeInterval)delayAmount {
    if (![self superview]) {
        return;
    }
