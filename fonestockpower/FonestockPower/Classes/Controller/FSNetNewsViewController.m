//
//  FSNetNewsViewController.m
//  FonestockPower
//
//  Created by Michael.Hsieh on 2015/1/15.
...
#define SECTORID 594
#define PARENTID 5
#define TITLENAME @"網路新聞"

@interface FSNetNewsViewController ()<UITableViewDataSource, UITableViewDelegate, UIScrollViewDelegate, UIActionSheetDelegate, FSRadioButtonSetDelegate>{
...
@implementation FSNetNewsViewController
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [dataModel.newsDataModel parseNewsUrlAndSaveToDB];
    
    if (!firstSend) {
        [dataModel.newsDataModel parseNewsContentUrlAndSaveToDB:SECTORID];
        [NewsObject sharedInstance].sectorID = SECTORID;
        firstSend = YES;
    }else{
        [dataModel.newsDataModel parseNewsContentUrlAndSaveToDB:[NewsObject sharedInstance].sectorID];
...
-(void)loadDB{
    ...
    btnNameArray = [[NSMutableArray alloc]initWithObjects:@"一般新聞", @"全球財金", @"股市新聞", @"盤勢分析", @"氣象概況", nil];
    ...
    for (int i = 0; i < [btnNameArray count]; i++){
        ...
        int strLen = (int)[[btnNameArray objectAtIndex:i] length];
...
- (void)radioButtonSet:(FSRadioButtonSet *)controller didSelectButtonAtIndex:(NSUInteger)selectedIndex{
    ...
    NSMutableArray *sectorIDArray = [[NSMutableArray alloc]initWithObjects:@"594", @"595", @"596", @"597" ,@"172", nil];
    
    [dataModel.newsDataModel parseNewsContentUrlAndSaveToDB:[(NSNumber *)[sectorIDArray objectAtIndex:selectedIndex]intValue]];
    
    
    [NewsObject sharedInstance].sectorID = [(NSNumber *)[sectorIDArray objectAtIndex:selectedIndex]intValue];

    navName = [btnNameArray objectAtIndex:selectedIndex];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.datetimeLabel.text = newObj.urlContentTime;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    FSNewsContentViewController *contentView = [[FSNewsContentViewController alloc]initWithNewsIndex:(int)indexPath.row + 1 array:tableViewArray net:YES name:navName];
