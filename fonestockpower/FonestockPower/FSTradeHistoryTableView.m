...
@implementation FSTradeHistoryTableView
- (void)setDelegate:(id<FSTradeHistoryTableViewDelegate>)delegate {
    _delegate = delegate;
}
...
- (void)updateConstraints {
    ...
    int mainViewWidth = _mainColumnSize * 4 + 80;
...
-(UIView *)MaintableViewForHeader{
    ...
    for (int i = 0; i < [columnsSecondName count]; i++) {
        UILabel *label = [[UILabel alloc] init];
        if (i == 0) {
            label.frame = CGRectMake(_mainColumnSize, 0, _mainColumnSize, 44);
        }
    ...
    for (int i = 0; i < [columnsThirdName count]; i++) {
        UILabel *label = [[UILabel alloc] init];
        if (i == 0) {
            label.frame = CGRectMake(_mainColumnSize * 2, 0, _mainColumnSize+10, 22);
        }else{
            label.frame = CGRectMake(_mainColumnSize * 2, 22, _mainColumnSize+10, 22);
        }
        label.numberOfLines = 0;
        [label setAdjustsFontSizeToFitWidth:YES];
        label.lineBreakMode = NSLineBreakByCharWrapping;
    ...
    for (int i = 0; i < [columnsThirdName count]; i++) {
        UILabel *label = [[UILabel alloc] init];
        if (i == 0) {
            label.frame = CGRectMake(_mainColumnSize * 3+10, 0, _mainColumnSize+10, 22);
        }else{
            label.frame = CGRectMake(_mainColumnSize * 3+10, 22, _mainColumnSize+10, 22);
        }
        label.numberOfLines = 0;
        [label setAdjustsFontSizeToFitWidth:YES];
        label.lineBreakMode = NSLineBreakByCharWrapping;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([tableView isEqual:_fixedTableView]) {
        FSTradeHistoryFixedTableViewCell *cell = nil;
        static NSString *CellIdentifier;
        
        cell = (FSTradeHistoryFixedTableViewCell *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        
        if (cell == nil) {
            NSArray *columnsName = [_delegate columnsInFixedTableView];
            cell = [[FSTradeHistoryFixedTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier AndLabelAmount:(int)[columnsName count]];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
        [_delegate updateFixedTableViewCellDateLabel:cell.dateLabel cellForRowAtIndexPath:indexPath cell:cell];
        return cell;
        
    } else if ([tableView isEqual:_mainTableView]) {
        FSTradeHistoryMainTableViewCell *cell = nil;
        static NSString *CellIdentifier;
        
        cell = (FSTradeHistoryMainTableViewCell *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        
        if (cell == nil) {
            cell = [[FSTradeHistoryMainTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier AndLabelAmount:5];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
        [_delegate updateMainTableViewCellSymbolLabel:cell.symbolLabel qtyLabel:cell.qtyLabel buyDealLabel:cell.buyDealLabel buyAmountLabel:cell.buyAmountLabel sellDealLable:cell.sellDealLabel sellAmountLabel:cell.sellAmountLabel removeBtn:cell.removeBtn cellForRowAtIndexPath:indexPath cell:cell];
        return cell;
    }
