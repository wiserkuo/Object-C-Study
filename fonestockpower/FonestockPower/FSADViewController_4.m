...
@interface FSADViewController_4 (){
    FSShowResultAdObj *AdObj;
}

@end

@implementation FSADViewController_4
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(initView) name:@"4.jpg" object:nil];
}
...
-(void)initView
{
    ...
    if ([AdData count] > 0) {
        AdObj = [[adSystem getLoaclAdPlist] objectAtIndex:3];
        imageView.image = [UIImage imageWithContentsOfFile:[NSString stringWithFormat:@"%@/ADImage/4.jpg",[CodingUtil fonestockDocumentsPath]]];
