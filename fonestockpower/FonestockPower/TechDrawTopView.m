...
#define drawBottomHeight 10.0 //低點從三角形以上開始
//圓的直徑
#define radius 6
...
- (void)drawRect:(CGRect)rect {
    ...
    //算出虛線的間距
    double dashHeight = (self.frame.size.height-topAndBottomHeight*2-drawBottomHeight) / 6;
    //畫水平虛線
    for(int i = 0; i <=5; i++){
        CGContextMoveToPoint(context   ,2,self.frame.size.height-drawBottomHeight - i * dashHeight);
        CGContextAddLineToPoint(context,self.frame.size.width,self.frame.size.height-drawBottomHeight - i * dashHeight);
    }
    //bug#10085 wiser start
    CGContextMoveToPoint(context, 2, self.frame.size.height-drawBottomHeight);
    CGContextAddLineToPoint(context,self.frame.size.width,  self.frame.size.height-10);
    //bug#10085 wiser end
    CGContextMoveToPoint(context, 2, topAndBottomHeight*2);//bug#10767 start
    CGContextAddLineToPoint(context,self.frame.size.width,  topAndBottomHeight*2);
    //bug#10767 end
    ...
    TechObject *firstObject = [_dataArray objectAtIndex:0];//bug#10767 start
    ...
    for(int i = _startCount; i<endCount; i++){
        ...
        if(month != detailMonth){
            detailMonth = month;
            CGContextMoveToPoint(context, i * _widthRange - 1.5 +_widthRange, topAndBottomHeight*2);
            CGContextAddLineToPoint(context, i * _widthRange - 1.5 +_widthRange, self.frame.size.height-drawBottomHeight);
            CGContextStrokePath(context);
        }
    }
    //bug#10767 end
    ...
    //算出資料畫圖高度的比例
    heightRate = (self.frame.size.height - topAndBottomHeight * 2-drawBottomHeight) / (highPrice - lowPrice);
    ...
    for(int i = _startCount; i<endCount; i ++){
        ...
        //bug#10085 wiser start
        if ((i == (_dataArray.count - 4) || i == (_dataArray.count - 9) || (i == _dataArray.count - 19))) {
            CGContextBeginPath(context);
            CGContextMoveToPoint(context, i * _widthRange, self.frame.size.height-drawBottomHeight);
            ...
            if (i == (_dataArray.count - 9)) {
                CGContextSetFillColorWithColor(context, [UIColor brownColor].CGColor);
            }
            else
            if (i == (_dataArray.count - 19)) {
        ...
        if(!isnan(m1Value)){
            if(!first1Flag){
                CGPathMoveToPoint(pathM1, NULL, i * _widthRange + _widthRange, (self.frame.size.height-drawBottomHeight) - (m1Value - lowPrice) * heightRate);
            }else if(i > _startCount){
                CGPathAddLineToPoint(pathM1, NULL, i * _widthRange + _widthRange , (self.frame.size.height-drawBottomHeight) - (m1Value - lowPrice) * heightRate);
            }
            first1Flag = YES;
        }
        if(!isnan(m2Value)){
            if(!first2Flag){
                CGPathMoveToPoint(pathM2, NULL, i * _widthRange + _widthRange, (self.frame.size.height-drawBottomHeight) - (m2Value - lowPrice) * heightRate);
            }else if(i > _startCount){
                CGPathAddLineToPoint(pathM2, NULL, i * _widthRange + _widthRange , (self.frame.size.height-drawBottomHeight) - (m2Value - lowPrice) * heightRate);
            }
            first2Flag = YES;
        }
        if(!isnan(m3Value)){
            if(!first3Flag){
                CGPathMoveToPoint(pathM3, NULL, i * _widthRange + _widthRange, (self.frame.size.height-drawBottomHeight) - (m3Value - lowPrice) * heightRate);
            }else if(i > _startCount){
                CGPathAddLineToPoint(pathM3, NULL, i * _widthRange + _widthRange , (self.frame.size.height-drawBottomHeight) - (m3Value - lowPrice) * heightRate);
            }
            first3Flag = YES;
        }
        //K棒線
        CGPathMoveToPoint(pathKLine, NULL, i * _widthRange +_widthRange , (self.frame.size.height-drawBottomHeight) - (obj.low - lowPrice) * heightRate);
        CGPathAddLineToPoint(pathKLine, NULL, i * _widthRange+_widthRange, (self.frame.size.height-drawBottomHeight) - (obj.high - lowPrice) * heightRate);
...
//        }
        ...
        if((_startCount <= _circleStart && endCount >= _circleStart) || (_startCount <= _circleEnd && endCount >= _circleEnd) || _circleStart == 0 || _circleEnd == 0){
            if(obj.date == _startDate){
                ...
                if(_symbolType){
                    CGContextAddArc(context, i * _widthRange + _widthRange, (self.frame.size.height-drawBottomHeight) - (obj.low - lowPrice) * heightRate, radius + _obj.pinchValue, 0, 2*M_PI, 0);
                    _circleTouchX_Start = i * _widthRange + _widthRange;
                    _circleTouchY_Start = (self.frame.size.height-drawBottomHeight) - (obj.low - lowPrice) * heightRate;
                }else{
                    CGContextAddArc(context, i * _widthRange + _widthRange, (self.frame.size.height-drawBottomHeight) - (obj.high - lowPrice) * heightRate, radius + _obj.pinchValue, 0, 2*M_PI, 0);
                    _circleTouchX_Start = i * _widthRange + _widthRange;
                    _circleTouchY_Start = (self.frame.size.height-drawBottomHeight) - (obj.high - lowPrice) * heightRate;
                }
                CGContextSetLineDash(context, 0, length, 0);
            ...
            if(obj.date == _endDate){
                ...
                if(_symbolType){
                    ...
                    _circleTouchY_End = (self.frame.size.height-drawBottomHeight) - (obj.low - lowPrice) * heightRate;
                }else{
                    CGContextAddArc(context, i * _widthRange + _widthRange, (self.frame.size.height-drawBottomHeight) - (obj.high - lowPrice) * heightRate, radius + _obj.pinchValue, 0, 2*M_PI, 0);
                    _circleTouchX_End = i * _widthRange + _widthRange;
                    _circleTouchY_End = (self.frame.size.height-drawBottomHeight) - (obj.high - lowPrice) * heightRate;
                }
    ...
    for(int i = _startCount; i <endCount; i ++){
        ...
        if(obj.open > obj.last){
            [downColor set];
            CGContextMoveToPoint(context, i * _widthRange+_widthRange, (self.frame.size.height-drawBottomHeight) - (obj.open - lowPrice) * heightRate);
            CGContextAddLineToPoint(context, i * _widthRange+_widthRange, (self.frame.size.height-drawBottomHeight) - (obj.last - lowPrice) * heightRate);
        }else if(obj.open < obj.last){
            [upColor set];
            CGContextMoveToPoint(context, i * _widthRange+_widthRange, (self.frame.size.height-drawBottomHeight) - (obj.last - lowPrice) * heightRate);
            CGContextAddLineToPoint(context, i * _widthRange+_widthRange, (self.frame.size.height-drawBottomHeight) - (obj.open - lowPrice) * heightRate);
        }else{
            [[UIColor blueColor]set];
            CGContextMoveToPoint(context, i * _widthRange+_widthRange, (self.frame.size.height-drawBottomHeight) - (obj.last - lowPrice) * heightRate);
            
            if (obj.last == lowPrice) {
                CGContextAddLineToPoint(context, i * _widthRange+_widthRange, (self.frame.size.height-drawBottomHeight) - (obj.last - lowPrice + 0.05 + (obj.last - lowPrice) / 200) * heightRate);
            } else {
                CGContextAddLineToPoint(context, i * _widthRange+_widthRange, (self.frame.size.height-drawBottomHeight) - (obj.last - lowPrice + (obj.last - lowPrice) / 200) * heightRate);
            }
        ...
        if (circleInViewCount >= 1) {
            ...
            if(!(_m==0 && _b==0)){
                double y = _m * i + _b;
                if(!hLineFirstFlag){
                   // if(!(self.frame.size.height-topAndBottomHeight*2-drawBottomHeight) - (y - lowPrice) * heightRate > self.frame.size.height-topAndBottomHeight*2-drawBottomHeight){
                        CGPathMoveToPoint(pathHLine, NULL, i * _widthRange + _widthRange,
                                (self.frame.size.height-drawBottomHeight) - (y - lowPrice) * heightRate);
                        hLineFirstFlag = YES;
                   // }
                }else{
                 //   if(!(self.frame.size.height-topAndBottomHeight*2-drawBottomHeight) - (y - lowPrice) * heightRate >self.frame.size.height-topAndBottomHeight*2-drawBottomHeight){
                        CGPathAddLineToPoint(pathHLine, NULL, i * _widthRange + _widthRange,
                                (self.frame.size.height-drawBottomHeight) - (y - lowPrice) * heightRate);
                 //   }
                }
                ...
                if(endCount == [_dataArray count]){
                    ...
                    if(!hLineFirstFlag){
                   //     if(!((self.frame.size.height-topAndBottomHeight*2-drawBottomHeight) - (y - lowPrice) * heightRate > self.frame.size.height-drawBottomHeight )){
                            CGPathMoveToPoint(pathHLine, NULL, lastCount * _widthRange + _widthRange,
                                    (self.frame.size.height-drawBottomHeight) - (y - lowPrice) * heightRate);
                            hLineFirstFlag = YES;
                   //     }
                    }else{
                    //    if(!((self.frame.size.height-topAndBottomHeight*2-drawBottomHeight) - (y - lowPrice) * heightRate > self.frame.size.height-drawBottomHeight )){
                            CGPathAddLineToPoint(pathHLine, NULL, lastCount * _widthRange + _widthRange,
                                    (self.frame.size.height-drawBottomHeight) - (y - lowPrice) * heightRate);
                   //     }
                    }
                }
...
-(void)longPressHandler:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if(selectNum<[_dataArray count]){
        ...
        double yPoint;
        yPoint = (self.frame.size.height-drawBottomHeight) - (obj.last - lowPrice) * heightRate;
