...
@implementation FSTradeDiaryTableView

- (void)setDelegate:(id<FSTradeDiaryTableViewDelegate>)delegate {
    _delegate = delegate;
}
...
- (void)updateConstraints {
    ...
    int mainViewWidth = _mainColumnSize * 5;
...
-(UIView *)MaintableViewForHeader{
    ...
    for (int i = 0; i < [columnsThirdName count]; i++) {
        UILabel *label = [[UILabel alloc] init];
        label.frame = CGRectMake(_mainColumnSize * 2, 0, _mainColumnSize, 44);
        label.numberOfLines = 0;
        [label setAdjustsFontSizeToFitWidth:YES];
        label.lineBreakMode = NSLineBreakByCharWrapping;
        label.backgroundColor = [UIColor clearColor];
    ...
    for (int i = 0; i < [columnsFourthName count]; i++) {
        UILabel *label = [[UILabel alloc]init];
        if (i == 0) {
            label.frame = CGRectMake(_mainColumnSize * 3, 0, _mainColumnSize, 22);
        }else if (i == 1){
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([tableView isEqual:_fixedTableView]) {
        FSTradeDiaryFixedTableViewCell *cell = nil;
        static NSString *CellIdentifier;
        
        cell = (FSTradeDiaryFixedTableViewCell *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        
        if (cell == nil) {
            NSArray *columnsName = [_delegate columnsInFixedTableView];
            cell = [[FSTradeDiaryFixedTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier AndLabelAmount:(int)[columnsName count]];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
        [_delegate updateFixedTableCellSymbolLabel:cell.symbolLabel cellForRowAtIndexPath:indexPath cell:cell];
        return cell;
        
    } else if ([tableView isEqual:_mainTableView]) {
        FSTradeDiaryMainTableViewCell *cell = nil;
        static NSString *CellIdentifier;
        
        cell = (FSTradeDiaryMainTableViewCell *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        
        if (cell == nil) {
            cell = [[FSTradeDiaryMainTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier AndLabelAmount:5];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
        [_delegate updateMainTableCellqtyLabel:cell.qtyLabel avgSellLabel:cell.avgSellLabel sellAmountLabel:cell.sellAmountLabel gainLabel:cell.gainLabel gainPercentLabel:cell.gainPercentLabel avgCost:cell.avgCostLabel costAmountLabel:cell.costAmountLabel hBtn:cell.hBtn nBtn:cell.nBtn gBtn:cell.gBtn cellForRowAtIndexPath:indexPath cell:cell];
        return cell;
    }
