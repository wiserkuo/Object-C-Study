...
@property (strong, nonatomic) NSMutableArray * balance1Array;
@property (strong, nonatomic) NSMutableArray * balance2Array;
@property (strong, nonatomic) NSMutableArray * income1Array;
@property (strong, nonatomic) NSMutableArray * income2Array;
@property (strong, nonatomic) NSMutableArray * cashFlow1Array;
@property (strong, nonatomic) NSMutableArray * cashFlow2Array;
@property (strong, nonatomic) NSMutableArray * financialRatio1Array;
@property (strong, nonatomic) NSMutableArray * financialRatio2Array;
...
- (void)viewDidLoad
{
    ...
    self.balance1Array = [[NSMutableArray alloc]init];
    self.balance2Array = [[NSMutableArray alloc]init];
    self.income1Array = [[NSMutableArray alloc]init];
    self.income2Array = [[NSMutableArray alloc]init];
    self.cashFlow1Array = [[NSMutableArray alloc]init];
    self.cashFlow2Array = [[NSMutableArray alloc]init];
    self.financialRatio1Array = [[NSMutableArray alloc]init];
    self.financialRatio2Array = [[NSMutableArray alloc]init];
    symbol1Dic = [[NSMutableDictionary alloc]init];
    symbol2Dic = [[NSMutableDictionary alloc]init];
    self.totalArray = [[NSMutableArray alloc]initWithObjects:NSLocalizedStringFromTable(@"累計", @"Equity", nil),NSLocalizedStringFromTable(@"單季", @"Equity", nil), nil];
    self.typeArray= [[NSMutableArray alloc]initWithObjects:@"$",@"%",@"$+%", nil];
    _total = 1;
...
-(void)reloadData{
    ...
    if (_twoStock) {
        ...
        self.identSymbol2 = [_watchportfolio.portfolioItem getIdentCodeSymbol];
    }
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    _dataModal.indicator.twoStockCompare = _twoStock;
...
-(void)loadDefaultValue{
    ...
    [_financialRatio2Array removeAllObjects];
    
    
    _balance1Array = [dataModel.financeReportUS searchFinanceDataDateWithReportType:@"BalanceSheet" IdentCodeSymbol:[_watchportfolio.portfolioItem getIdentCodeSymbol]];
    if ([[_watchportfolio.portfolioItem getIdentCodeSymbol] isEqualToString:[_watchportfolio.comparedPortfolioItem getIdentCodeSymbol]] || !_twoStock) {
        _balance2Array = _balance1Array;
    }else{
        _balance2Array = [dataModel.financeReportUS searchFinanceDataDateWithReportType:@"BalanceSheet" IdentCodeSymbol:[_watchportfolio.comparedPortfolioItem getIdentCodeSymbol]];
    }
    
    _income1Array = [dataModel.financeReportUS searchFinanceDataDateWithReportType:@"IncomeStatement" IdentCodeSymbol:[_watchportfolio.portfolioItem getIdentCodeSymbol]];
    if ([[_watchportfolio.portfolioItem getIdentCodeSymbol] isEqualToString:[_watchportfolio.comparedPortfolioItem getIdentCodeSymbol]] || !_twoStock) {
        _income2Array = _income1Array;
    }else{
        _income2Array = [dataModel.financeReportUS searchFinanceDataDateWithReportType:@"IncomeStatement" IdentCodeSymbol:[_watchportfolio.comparedPortfolioItem getIdentCodeSymbol]];
    }
    
    _cashFlow1Array = [dataModel.financeReportUS searchFinanceDataDateWithReportType:@"CashFlow" IdentCodeSymbol:[_watchportfolio.portfolioItem getIdentCodeSymbol]];
    if ([[_watchportfolio.portfolioItem getIdentCodeSymbol] isEqualToString:[_watchportfolio.comparedPortfolioItem getIdentCodeSymbol]] || !_twoStock) {
        _cashFlow2Array = _cashFlow1Array;
    }else{
        _cashFlow2Array = [dataModel.financeReportUS searchFinanceDataDateWithReportType:@"CashFlow" IdentCodeSymbol:[_watchportfolio.comparedPortfolioItem getIdentCodeSymbol]];
    }
    
    _financialRatio1Array = [dataModel.financeReportUS searchFinanceDataDateWithReportType:@"FinancialRatio" IdentCodeSymbol:[_watchportfolio.portfolioItem getIdentCodeSymbol]];
    if ([[_watchportfolio.portfolioItem getIdentCodeSymbol] isEqualToString:[_watchportfolio.comparedPortfolioItem getIdentCodeSymbol]] || !_twoStock) {
        _financialRatio2Array = _financialRatio1Array;
        [self searchDataWithGroup:@"Group2"];
    }else{
        _financialRatio2Array = [dataModel.financeReportUS searchFinanceDataDateWithReportType:@"FinancialRatio" IdentCodeSymbol:[_watchportfolio.comparedPortfolioItem getIdentCodeSymbol]];
    }
        ...
        if([sheetTitle isEqualToString:@"BalanceSheet1"]){
            ...
            if (count>0) {
                ...
                for(int i=0 ; i<count ; i++){
                    [_balance1Array addObject:[NSNumber numberWithUnsignedInt:[dataModel.balanceSheet findDateByIdentSymbol:_identSymbol Postion:i]]];
                }
                if ([[_watchportfolio.portfolioItem getIdentCodeSymbol] isEqualToString:[_watchportfolio.comparedPortfolioItem getIdentCodeSymbol]] || !_twoStock) {
                    _balance2Array = _balance1Array;
                }
            ...
            if (count2>0) {
                ...
                for(int i=0 ; i<count2 ; i++){
                    [_balance2Array addObject:[NSNumber numberWithUnsignedInt:[dataModel.balanceSheet findDateByIdentSymbol:_identSymbol2 Postion:i]]];
                }
            }
            [_balanceTableView reloadData];
            ...
            if (count>0) {
                ...
                for(int i=0 ; i<count ; i++){
                    [_income1Array addObject:[NSNumber numberWithUnsignedInt:[dataModel.income findDateByIdentSymbol:_identSymbol Postion:i]]];
                }
                if ([[_watchportfolio.portfolioItem getIdentCodeSymbol] isEqualToString:[_watchportfolio.comparedPortfolioItem getIdentCodeSymbol]] || !_twoStock) {
                    _income2Array = _income1Array;
                }
            ...
            if (count>0) {
                ...
                for(int i=0 ; i<count ; i++){
                    [_income2Array addObject:[NSNumber numberWithUnsignedInt:[dataModel.income findDateByIdentSymbol:_identSymbol2 Postion:i]]];
                }
            }
            ...
            if (count>0) {
                ...
                for(int i=0 ; i<count ; i++){
                    [_cashFlow1Array addObject:[NSNumber numberWithUnsignedInt:[dataModel.cashFlow findDateByIdentSymbol:_identSymbol Postion:i]]];
                }
                if ([[_watchportfolio.portfolioItem getIdentCodeSymbol] isEqualToString:[_watchportfolio.comparedPortfolioItem getIdentCodeSymbol]] || !_twoStock) {
                    _cashFlow2Array = _cashFlow1Array;
                }
            ...
            if (count>0) {
                ...
                for(int i=0 ; i<count ; i++){
                    [_cashFlow2Array addObject:[NSNumber numberWithUnsignedInt:[dataModel.cashFlow findDateByIdentSymbol:_identSymbol2 Postion:i]]];
                }
            }
            ...
            if (count>0) {
                ...
                for(int i=0 ; i<count ; i++){
                    [_financialRatio1Array addObject:[NSNumber numberWithUnsignedInt:[dataModel.financialRatio findDateByIdentSymbol:_identSymbol Postion:i]]];
                }
                if ([[_watchportfolio.portfolioItem getIdentCodeSymbol] isEqualToString:[_watchportfolio.comparedPortfolioItem getIdentCodeSymbol]] || !_twoStock) {
                    _financialRatio2Array = _financialRatio1Array;
                }
            ...
            if ([group isEqualToString:@"tw"])
            {
                ...
                NSMutableDictionary * tmpDic = [_dataModal.income getDataDictionaryWithIdentSymbol:_identSymbol];
                [dic setObject:tmpDic forKey:@"incomeValueDict"];
                
                tmpDic = [_dataModal.balanceSheet getDataDictionaryWithIdentSymbol:_identSymbol];
                [dic setObject:tmpDic forKey:@"balanceValueDict"];
                
                [_dataModal.financialRatio calculateWithWithIdentSymbol:_identSymbol Index:_financialRatioPostion Data:dic Date:[(NSNumber *)[_financialRatio1Array objectAtIndex:_financialRatioPostion]intValue]];
                
                if ([[_watchportfolio.portfolioItem getIdentCodeSymbol] isEqualToString:[_watchportfolio.comparedPortfolioItem getIdentCodeSymbol]] || !_twoStock) {
                    NSMutableDictionary * dic = [[NSMutableDictionary alloc]init];
                    NSMutableDictionary * tmpDic = [_dataModal.income getDataDictionaryWithIdentSymbol:_identSymbol2];
                    [dic setObject:tmpDic forKey:@"incomeValueDict"];
                    
                    tmpDic = [_dataModal.balanceSheet getDataDictionaryWithIdentSymbol:_identSymbol2];
                    [dic setObject:tmpDic forKey:@"balanceValueDict"];
                    
                    [_dataModal.financialRatio calculateWithWithIdentSymbol:_identSymbol2 Index:_financialRatioPostion2 Data:dic Date:[(NSNumber *)[_financialRatio2Array objectAtIndex:_financialRatioPostion2]intValue]];
                }
            }
            [_financialRatioTableView reloadData];
            ...
            if (count>0) {
                ...
                for(int i=0 ; i<count ; i++){
                    [_financialRatio2Array addObject:[NSNumber numberWithUnsignedInt:[dataModel.financialRatio findDateByIdentSymbol:_identSymbol2 Postion:i]]];
                }
            }
...
-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    if ([actionSheet isEqual:_totalActionSheet]) {
        ...
        if (buttonIndex==0) {
            _total = 0;
            _dataModal.income.total = YES;
            _dataModal.cashFlow.total = YES;
            _dataModal.financialRatio.total = YES;
...
//            _financialRatioHeaderTitleLabel.text = NSLocalizedStringFromTable(@"財報日期(累計)", @"Equity", nil);
            ...
            _total = 1;
            _dataModal.income.total = NO;
            _dataModal.cashFlow.total = NO;
            _dataModal.financialRatio.total = NO;
...
//            _financialRatioHeaderTitleLabel.text = NSLocalizedStringFromTable(@"財報日期(單季)", @"Equity", nil);
        ...
        if (buttonIndex != 2) {
            ...
            NSMutableDictionary * dic = [[NSMutableDictionary alloc]init];
            NSMutableDictionary * tmpDic = [_dataModal.income getDataDictionaryWithIdentSymbol:_identSymbol];
            UInt16 date = [(NSNumber *)[_financialRatio1Array objectAtIndex:_financialRatioPostion]intValue];
            [dic setObject:tmpDic forKey:@"incomeValueDict"];
            
            tmpDic = [_dataModal.balanceSheet getDataDictionaryWithIdentSymbol:_identSymbol];
            [dic setObject:tmpDic forKey:@"balanceValueDict"];
            [_dataModal.financialRatio calculateWithWithIdentSymbol:_identSymbol Index:_financialRatioPostion Data:dic Date:date];
            [dic removeAllObjects];
            
            tmpDic = [_dataModal.income getDataDictionaryWithIdentSymbol:_identSymbol2];
            date = [(NSNumber *)[_financialRatio2Array objectAtIndex:_financialRatioPostion2]intValue];
            [dic setObject:tmpDic forKey:@"incomeValueDict"];
            
            tmpDic = [_dataModal.balanceSheet getDataDictionaryWithIdentSymbol:_identSymbol2];
            [dic setObject:tmpDic forKey:@"balanceValueDict"];
            [_dataModal.financialRatio calculateWithWithIdentSymbol:_identSymbol2 Index:_financialRatioPostion2 Data:dic Date:date];
        ...
        [_incomeTableView reloadData];
        [_cashFlowTableView reloadData];
    }else if ([actionSheet isEqual:_group1ActionSheet] && buttonIndex < [_balance1Array count]){
        NSMutableDictionary * dic = [[NSMutableDictionary alloc]init];
        UInt16 date;
        if ([_group1Btn isEqual:_balanceGroup1Btn]) {
            _balancePostion = (int)buttonIndex;
            date = [(NSNumber *)[_balance1Array objectAtIndex:buttonIndex]intValue];
            NSMutableDictionary * tmpDic = [_dataModal.income getDataDictionaryWithIdentSymbol:_identSymbol];
            [dic setObject:tmpDic forKey:@"incomeValueDict"];
            ...
            _incomePostion = [self setPostionWithPostion:self.balancePostion SearchArray:self.balance1Array TargetArray:self.income1Array];
            _cashFlowPostion = [self setPostionWithPostion:self.balancePostion SearchArray:self.balance1Array TargetArray:self.cashFlow1Array];
            _financialRatioPostion = [self setPostionWithPostion:self.balancePostion SearchArray:self.balance1Array TargetArray:self.financialRatio1Array];
        }else if ([_group1Btn isEqual:_incomeGroup1Btn] && buttonIndex < [_income1Array count]){
            _incomePostion = (int)buttonIndex;
            NSMutableDictionary * tmpDic = [_dataModal.income getDataDictionaryWithIdentSymbol:_identSymbol];
            date = [(NSNumber *)[_income1Array objectAtIndex:buttonIndex]intValue];
            [dic setObject:tmpDic forKey:@"incomeValueDict"];
            
            tmpDic = [_dataModal.balanceSheet getDataDictionaryWithIdentSymbol:_identSymbol];
            [dic setObject:tmpDic forKey:@"balanceValueDict"];
            
            _balancePostion = [self setPostionWithPostion:self.incomePostion SearchArray:self.income1Array TargetArray:self.balance1Array];
            _cashFlowPostion = [self setPostionWithPostion:self.incomePostion SearchArray:self.income1Array TargetArray:self.cashFlow1Array];
            _financialRatioPostion = [self setPostionWithPostion:self.incomePostion SearchArray:self.income1Array TargetArray:self.financialRatio1Array];
        }else if ([_group1Btn isEqual:_cashFlowGroup1Btn] && buttonIndex < [_cashFlow1Array count]){
            self.cashFlowPostion = (int)buttonIndex;
            NSMutableDictionary * tmpDic = [_dataModal.income getDataDictionaryWithIdentSymbol:_identSymbol];
            date = [(NSNumber *)[_cashFlow1Array objectAtIndex:buttonIndex]intValue];
            [dic setObject:tmpDic forKey:@"incomeValueDict"];
            
            tmpDic = [_dataModal.balanceSheet getDataDictionaryWithIdentSymbol:_identSymbol];
            [dic setObject:tmpDic forKey:@"balanceValueDict"];
            
            self.balancePostion = [self setPostionWithPostion:self.cashFlowPostion SearchArray:self.cashFlow1Array TargetArray:self.balance1Array];
            self.incomePostion = [self setPostionWithPostion:self.cashFlowPostion SearchArray:self.cashFlow1Array TargetArray:self.income1Array];
            self.financialRatioPostion = [self setPostionWithPostion:self.cashFlowPostion SearchArray:self.cashFlow1Array TargetArray:self.financialRatio1Array];

        }else if ([_group1Btn isEqual:_financialRatioGroup1Btn] && buttonIndex < [_financialRatio1Array count]){
            self.financialRatioPostion = (int)buttonIndex;
            NSMutableDictionary * tmpDic = [_dataModal.income getDataDictionaryWithIdentSymbol:_identSymbol];
            date = [(NSNumber *)[_financialRatio1Array objectAtIndex:buttonIndex]intValue];
            [dic setObject:tmpDic forKey:@"incomeValueDict"];
            
            tmpDic = [_dataModal.balanceSheet getDataDictionaryWithIdentSymbol:_identSymbol];
            [dic setObject:tmpDic forKey:@"balanceValueDict"];
            
            self.balancePostion = [self setPostionWithPostion:self.financialRatioPostion SearchArray:self.financialRatio1Array TargetArray:self.balance1Array];
            self.incomePostion = [self setPostionWithPostion:self.financialRatioPostion SearchArray:self.financialRatio1Array TargetArray:self.income1Array];
            self.cashFlowPostion = [self setPostionWithPostion:self.financialRatioPostion SearchArray:self.financialRatio1Array TargetArray:self.cashFlow1Array];
        }
        NSString * appid = [FSFonestock sharedInstance].appId;
        NSString * group = [appid substringWithRange:NSMakeRange(0, 2)];
        if ([group isEqualToString:@"us"])
        ...
        NSMutableDictionary * dic = [[NSMutableDictionary alloc]init];
        UInt16 date;
        if ([_group2Btn isEqual:self.balanceGroup2Btn] && buttonIndex < [_balance2Array count]) {
            self.balancePostion2 = (int)buttonIndex;
            NSMutableDictionary * tmpDic = [_dataModal.income getDataDictionaryWithIdentSymbol:_identSymbol2];
            date = [(NSNumber *)[_balance2Array objectAtIndex:buttonIndex]intValue];
            [dic setObject:tmpDic forKey:@"incomeValueDict"];
            
            tmpDic = [_dataModal.balanceSheet getDataDictionaryWithIdentSymbol:_identSymbol2];
            [dic setObject:tmpDic forKey:@"balanceValueDict"];

            self.incomePostion2 = [self setPostionWithPostion:self.balancePostion2 SearchArray:self.balance2Array TargetArray:self.income2Array];
            self.cashFlowPostion2 = [self setPostionWithPostion:self.balancePostion2 SearchArray:self.balance2Array TargetArray:self.cashFlow2Array];
            self.financialRatioPostion2 = [self setPostionWithPostion:self.balancePostion2 SearchArray:self.balance2Array TargetArray:self.financialRatio2Array];
        }else if ([_group2Btn isEqual:self.incomeGroup2Btn] && buttonIndex < [_income2Array count]){
            self.incomePostion2 = (int)buttonIndex;
            NSMutableDictionary * tmpDic = [_dataModal.income getDataDictionaryWithIdentSymbol:_identSymbol2];
            date = [(NSNumber *)[_income2Array objectAtIndex:buttonIndex]intValue];
            [dic setObject:tmpDic forKey:@"incomeValueDict"];
            
            tmpDic = [_dataModal.balanceSheet getDataDictionaryWithIdentSymbol:_identSymbol2];
            [dic setObject:tmpDic forKey:@"balanceValueDict"];
            
            self.balancePostion2 = [self setPostionWithPostion:self.incomePostion2 SearchArray:self.income2Array TargetArray:self.balance2Array];
            self.cashFlowPostion2 = [self setPostionWithPostion:self.incomePostion2 SearchArray:self.income2Array TargetArray:self.cashFlow2Array];
            self.financialRatioPostion2 = [self setPostionWithPostion:self.incomePostion2 SearchArray:self.income2Array TargetArray:self.financialRatio2Array];
        }else if ([_group2Btn isEqual:self.cashFlowGroup2Btn] && buttonIndex < [_cashFlow2Array count]){
            self.cashFlowPostion2 = (int)buttonIndex;
            NSMutableDictionary * tmpDic = [_dataModal.income getDataDictionaryWithIdentSymbol:_identSymbol2];
            date = [(NSNumber *)[_cashFlow2Array objectAtIndex:buttonIndex]intValue];
            ...
            self.balancePostion2 = [self setPostionWithPostion:self.cashFlowPostion2 SearchArray:self.cashFlow2Array TargetArray:self.balance2Array];
            self.incomePostion2 = [self setPostionWithPostion:self.cashFlowPostion2 SearchArray:self.cashFlow2Array TargetArray:self.income2Array];
            self.financialRatioPostion2 = [self setPostionWithPostion:self.cashFlowPostion2 SearchArray:self.cashFlow2Array TargetArray:self.financialRatio2Array];
        }else if ([_group2Btn isEqual:self.financialRatioGroup2Btn] && buttonIndex < [_financialRatio2Array count]){
            self.financialRatioPostion2 = (int)buttonIndex;
            NSMutableDictionary * tmpDic = [_dataModal.income getDataDictionaryWithIdentSymbol:_identSymbol2];
            date = [(NSNumber *)[_financialRatio2Array objectAtIndex:buttonIndex]intValue];
            [dic setObject:tmpDic forKey:@"incomeValueDict"];
            
            tmpDic = [_dataModal.balanceSheet getDataDictionaryWithIdentSymbol:_identSymbol2];
            [dic setObject:tmpDic forKey:@"balanceValueDict"];
            
            self.balancePostion2 = [self setPostionWithPostion:self.financialRatioPostion2 SearchArray:self.financialRatio2Array TargetArray:self.balance2Array];
            self.incomePostion2 = [self setPostionWithPostion:self.financialRatioPostion2 SearchArray:self.financialRatio2Array TargetArray:self.income2Array];
            self.cashFlowPostion2 = [self setPostionWithPostion:self.financialRatioPostion2 SearchArray:self.financialRatio2Array TargetArray:self.cashFlow2Array];
        }
        NSString * appid = [FSFonestock sharedInstance].appId;
        NSString * group = [appid substringWithRange:NSMakeRange(0, 2)];
        if ([group isEqualToString:@"us"])
...
-(void)searchDataWithGroup:(NSString *)gp{
    ...
    if ([gp isEqualToString:@"Group1"]) {
        ...
        if (_balancePostion < [_balance1Array count]) {
            [self searchWithIds:[_watchportfolio.portfolioItem getIdentCodeSymbol] ReportType:@"BalanceSheet" date:[(NSNumber *)[_balance1Array objectAtIndex:_balancePostion] intValue] dataDic:symbol1Dic];
        }
        if (_incomePostion < [_income1Array count]) {
            [self searchWithIds:[_watchportfolio.portfolioItem getIdentCodeSymbol] ReportType:@"IncomeStatement" date:[(NSNumber *)[_income1Array objectAtIndex:_incomePostion] intValue] dataDic:symbol1Dic];
        }
        if (_cashFlowPostion < [_cashFlow1Array count]) {
            [self searchWithIds:[_watchportfolio.portfolioItem getIdentCodeSymbol] ReportType:@"CashFlow" date:[(NSNumber *)[_cashFlow1Array objectAtIndex:_cashFlowPostion] intValue] dataDic:symbol1Dic];
        }
        if (_financialRatioPostion < [_financialRatio1Array count]) {
            [self searchWithIds:[_watchportfolio.portfolioItem getIdentCodeSymbol] ReportType:@"FinancialRatio" date:[(NSNumber *)[_financialRatio1Array objectAtIndex:_financialRatioPostion] intValue] dataDic:symbol1Dic];
        }
    }else if ([gp isEqualToString:@"Group2"]){
        [symbol2Dic removeAllObjects];
        if (_balancePostion2 < [_balance2Array count]) {
            [self searchWithIds:_identSymbol2 ReportType:@"BalanceSheet" date:[(NSNumber *)[_balance2Array objectAtIndex:_balancePostion2] intValue] dataDic:symbol2Dic];
        }
        if (_incomePostion2 < [_income2Array count]) {
            [self searchWithIds:_identSymbol2 ReportType:@"IncomeStatement" date:[(NSNumber *)[_income2Array objectAtIndex:_incomePostion2] intValue] dataDic:symbol2Dic];
        }
        if (_cashFlowPostion2 < [_cashFlow2Array count]) {
            [self searchWithIds:_identSymbol2 ReportType:@"CashFlow" date:[(NSNumber *)[_cashFlow2Array objectAtIndex:_cashFlowPostion2] intValue] dataDic:symbol2Dic];
        }
        if (_financialRatioPostion2 < [_financialRatio2Array count]) {
            [self searchWithIds:_identSymbol2 ReportType:@"FinancialRatio" date:[(NSNumber *)[_financialRatio2Array objectAtIndex:_financialRatioPostion2] intValue] dataDic:symbol2Dic];
        }
...
-(void)typeBtnClick:(FSUIButton*)btn{
    ...
    int count = [_dataModal.balanceSheet getDateCountByIdentSymbol:[_watchportfolio.portfolioItem getIdentCodeSymbol]];
    
    if (count>0) {
        self.symbol1EndDate =[_dataModal.balanceSheet findDateByIdentSymbol:_identSymbol Postion:0];
        self.symbol1BeginDate =[_dataModal.balanceSheet findDateByIdentSymbol:_identSymbol Postion:count-1];
        if (count>8) {
            count =8;
            self.symbol1BeginDate =[_dataModal.balanceSheet findDateByIdentSymbol:_identSymbol Postion:7];
        }
        for(int i=0 ; i<count ; i++){
            [_balance1Array addObject:[NSNumber numberWithUnsignedInt:[_dataModal.balanceSheet findDateByIdentSymbol:_identSymbol Postion:i]]];
        }
        if ([[_watchportfolio.portfolioItem getIdentCodeSymbol] isEqualToString:[_watchportfolio.comparedPortfolioItem getIdentCodeSymbol]] || !_twoStock) {
            _balance2Array = _balance1Array;
        }
    ...
    [_balance2Array removeAllObjects];
    int count2 = [_dataModal.balanceSheet getDateCountByIdentSymbol:_identSymbol2];
    
    if (count2>0) {
        self.symbol2EndDate =[_dataModal.balanceSheet findDateByIdentSymbol:_identSymbol2 Postion:0];
        self.symbol2BeginDate =[_dataModal.balanceSheet findDateByIdentSymbol:_identSymbol2 Postion:count2-1];
        if (count2>8) {
            count2 =8;
            self.symbol2BeginDate =[_dataModal.balanceSheet findDateByIdentSymbol:_identSymbol2 Postion:7];
        }
        for(int i=0 ; i<count2 ; i++){
            [_balance2Array addObject:[NSNumber numberWithUnsignedInt:[_dataModal.balanceSheet findDateByIdentSymbol:_identSymbol2 Postion:i]]];
        }
    }
    [_balanceTableView reloadData];
    
    [_income1Array removeAllObjects];
    count = [_dataModal.income getDateCountByIdentSymbol:_identSymbol];
    if (count>0) {
        if (count>8) {
            count =8;
        }
        for(int i=0 ; i<count ; i++){
            [_income1Array addObject:[NSNumber numberWithUnsignedInt:[_dataModal.income findDateByIdentSymbol:_identSymbol Postion:i]]];
        }
        if ([[_watchportfolio.portfolioItem getIdentCodeSymbol] isEqualToString:[_watchportfolio.comparedPortfolioItem getIdentCodeSymbol]] || !_twoStock) {
            _income2Array = _income1Array;
        }
    ...
    [_income2Array removeAllObjects];
    count = [_dataModal.income getDateCountByIdentSymbol:_identSymbol2];
    if (count>0) {
        if (count>8) {
            count =8;
        }
        for(int i=0 ; i<count ; i++){
            [_income2Array addObject:[NSNumber numberWithUnsignedInt:[_dataModal.income findDateByIdentSymbol:_identSymbol2 Postion:i]]];
        }
    }
    [_incomeTableView reloadData];
    ...
    [_cashFlow1Array removeAllObjects];
    count = [_dataModal.cashFlow getDateCountByIdentSymbol:_identSymbol];
    if (count>0) {
        if (count>8) {
            count =8;
        }
        for(int i=0 ; i<count ; i++){
            [_cashFlow1Array addObject:[NSNumber numberWithUnsignedInt:[_dataModal.cashFlow findDateByIdentSymbol:_identSymbol Postion:i]]];
        }
        if ([[_watchportfolio.portfolioItem getIdentCodeSymbol] isEqualToString:[_watchportfolio.comparedPortfolioItem getIdentCodeSymbol]] || !_twoStock) {
            _cashFlow2Array = _cashFlow1Array;
        }
    ...
    [_cashFlow2Array removeAllObjects];
    count = [_dataModal.cashFlow getDateCountByIdentSymbol:_identSymbol2];
    if (count>0) {
        if (count>8) {
            count =8;
        }
        for(int i=0 ; i<count ; i++){
            [_cashFlow2Array addObject:[NSNumber numberWithUnsignedInt:[_dataModal.cashFlow findDateByIdentSymbol:_identSymbol2 Postion:i]]];
        }
    }
    [_cashFlowTableView reloadData];
}
...
//    BOOL mergeBool = !btn.selected;
    ...
    _dataModal.balanceSheet.merge = btn.selected;
    _dataModal.income.merge = btn.selected;
    _dataModal.cashFlow.merge = btn.selected;
    _dataModal.financialRatio.merge = btn.selected;
    [_balance1Array removeAllObjects];
    int count = [_dataModal.balanceSheet getDateCountByIdentSymbol:[_watchportfolio.portfolioItem getIdentCodeSymbol]];
    ...
    [_financialRatio1Array removeAllObjects];
    count = [_dataModal.financialRatio getDateCountByIdentSymbol:_identSymbol];
    if (count>0) {
        if (count>8) {
            count =8;
        }
        for(int i=0 ; i<count ; i++){
            [_financialRatio1Array addObject:[NSNumber numberWithUnsignedInt:[_dataModal.financialRatio findDateByIdentSymbol:_identSymbol Postion:i]]];
        }
        if ([[_watchportfolio.portfolioItem getIdentCodeSymbol] isEqualToString:[_watchportfolio.comparedPortfolioItem getIdentCodeSymbol]] || !_twoStock) {
            _financialRatio2Array = _financialRatio1Array;
        }
    ...
    [_financialRatio2Array removeAllObjects];
    count = [_dataModal.financialRatio getDateCountByIdentSymbol:_identSymbol2];
    if (count>0) {
        if (count>8) {
            count =8;
        }
        for(int i=0 ; i<count ; i++){
            [_financialRatio2Array addObject:[NSNumber numberWithUnsignedInt:[_dataModal.financialRatio findDateByIdentSymbol:_identSymbol2 Postion:i]]];
        }
    }
    NSMutableDictionary * dic = [[NSMutableDictionary alloc]init];
    NSMutableDictionary * tmpDic = [_dataModal.income getDataDictionaryWithIdentSymbol:_identSymbol];
...
-(void)initBalanceTitle{
    ...
    FSUIButton * balanceTwoStockBtn = [[FSUIButton alloc]initWithButtonType:FSUIButtonTypeNormalBlue];
...
-(void)initIncomeTitle{
    ...
    FSUIButton * incomeTwoStockBtn = [[FSUIButton alloc]initWithButtonType:FSUIButtonTypeNormalBlue];
...
-(void)initCashFlowTitle{
    ...
    FSUIButton * cashFlowTwoStockBtn = [[FSUIButton alloc]initWithButtonType:FSUIButtonTypeNormalBlue];
...
-(void)initFinancialRatioTitle{
    ...
    FSUIButton * financialRatioTwoStockBtn = [[FSUIButton alloc]initWithButtonType:FSUIButtonTypeNormalBlue];
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    if (section == 0){
        ...
        if ([tableView isEqual:_balanceTableView]) {
            ...
            NSDateFormatter *dateFormat = [[NSDateFormatter alloc] init];
            if (!([_balance1Array count]==0) && _balancePostion<8) {
                [dateFormat setDateFormat:@"yyyy"];
                NSString *year = [dateFormat stringFromDate:[[_balance1Array objectAtIndex:_balancePostion] uint16ToDate]];
                [dateFormat setDateFormat:@"MM"];
                NSString * month =[dateFormat stringFromDate:[[_balance1Array objectAtIndex:_balancePostion] uint16ToDate]];
                NSString * Q = [self dateToQuarter:month];
                if (item->identCode[0]=='T' && item->identCode[1]=='W') {
                    NSString * dateString = [NSString stringWithFormat:@"%d-%@",[year intValue]-1911,Q];
            ...
            if (![_balance2Array count]==0) {
                NSString *year;
                NSString * month;
                if ([_balance2Array count]>_balancePostion2){
                    [dateFormat setDateFormat:@"yyyy"];
                    year= [dateFormat stringFromDate:[[_balance2Array objectAtIndex:_balancePostion2] uint16ToDate]];
                    [dateFormat setDateFormat:@"MM"];
                    month=[dateFormat stringFromDate:[[_balance2Array objectAtIndex:_balancePostion2] uint16ToDate]];
                }else{
                    [dateFormat setDateFormat:@"yyyy"];
                    year= [dateFormat stringFromDate:[[_balance2Array lastObject] uint16ToDate]];
                    [dateFormat setDateFormat:@"MM"];
                    month=[dateFormat stringFromDate:[[_balance2Array lastObject] uint16ToDate]];
                    _balancePostion2 = (int)[_balance2Array count]-1;
                }
                NSString * Q = [self dateToQuarter:month];
                if (item->identCode[0]=='T' && item->identCode[1]=='W') {
            ...
            if (!([_income1Array count]==0)) {
                [dateFormat setDateFormat:@"yyyy"];
                NSString *year = [dateFormat stringFromDate:[[_income1Array objectAtIndex:_incomePostion] uint16ToDate]];
                [dateFormat setDateFormat:@"MM"];
                NSString * month =[dateFormat stringFromDate:[[_income1Array objectAtIndex:_incomePostion] uint16ToDate]];
                NSString * Q = [self dateToQuarter:month];
                if (item->identCode[0]=='T' && item->identCode[1]=='W') {
            ...
            if (!([_income2Array count]==0)) {
                
                NSString *year;
                NSString * month;
                if ([_income2Array count]>_incomePostion2){
                    [dateFormat setDateFormat:@"yyyy"];
                     year= [dateFormat stringFromDate:[[_income2Array objectAtIndex:_incomePostion2] uint16ToDate]];
                    [dateFormat setDateFormat:@"MM"];
                    month=[dateFormat stringFromDate:[[_income2Array objectAtIndex:_incomePostion2] uint16ToDate]];
                }else{
                    [dateFormat setDateFormat:@"yyyy"];
                    year= [dateFormat stringFromDate:[[_income2Array lastObject] uint16ToDate]];
                    [dateFormat setDateFormat:@"MM"];
                    month=[dateFormat stringFromDate:[[_income2Array lastObject] uint16ToDate]];
                     _incomePostion2 = (int)[_income2Array count]-1;
                }
            ...
            NSDateFormatter *dateFormat = [[NSDateFormatter alloc] init];
            [dateFormat setDateFormat:@"yyyy/MM"];
            if (!([_cashFlow1Array count]==0)) {
                [dateFormat setDateFormat:@"yyyy"];
                NSString *year = [dateFormat stringFromDate:[[_cashFlow1Array objectAtIndex:_cashFlowPostion] uint16ToDate]];
                [dateFormat setDateFormat:@"MM"];
                NSString * month =[dateFormat stringFromDate:[[_cashFlow1Array objectAtIndex:_cashFlowPostion] uint16ToDate]];
                NSString * Q = [self dateToQuarter:month];
                if (item->identCode[0]=='T' && item->identCode[1]=='W') {
            ...
            if (!([_cashFlow2Array count]==0)) {
                NSString *year;
                NSString * month;
                if ([_cashFlow2Array count]>_cashFlowPostion2){
                    [dateFormat setDateFormat:@"yyyy"];
                    year= [dateFormat stringFromDate:[[_cashFlow2Array objectAtIndex:_cashFlowPostion2] uint16ToDate]];
                    [dateFormat setDateFormat:@"MM"];
                    month=[dateFormat stringFromDate:[[_cashFlow2Array objectAtIndex:_cashFlowPostion2] uint16ToDate]];
                }else{
                    [dateFormat setDateFormat:@"yyyy"];
                    year= [dateFormat stringFromDate:[[_cashFlow2Array lastObject] uint16ToDate]];
                    [dateFormat setDateFormat:@"MM"];
                    month=[dateFormat stringFromDate:[[_cashFlow2Array lastObject] uint16ToDate]];
                    _cashFlowPostion2 = (int)[_cashFlow2Array count]-1;
                }
                NSString * Q = [self dateToQuarter:month];
                if (item->identCode[0]=='T' && item->identCode[1]=='W') {
            ...
            NSDateFormatter *dateFormat = [[NSDateFormatter alloc] init];
            [dateFormat setDateFormat:@"yyyy/MM"];
            if (!([_financialRatio1Array count]==0)) {
                [dateFormat setDateFormat:@"yyyy"];
                NSString *year = [dateFormat stringFromDate:[[_financialRatio1Array objectAtIndex:_financialRatioPostion] uint16ToDate]];
                [dateFormat setDateFormat:@"MM"];
                NSString * month =[dateFormat stringFromDate:[[_financialRatio1Array objectAtIndex:_financialRatioPostion] uint16ToDate]];
                NSString * Q = [self dateToQuarter:month];
                if (item->identCode[0]=='T' && item->identCode[1]=='W') {
            ...
            if (!([_financialRatio2Array count]==0)) {
                NSString *year;
                NSString * month;
                if ([_financialRatio2Array count]>_financialRatioPostion2){
                    [dateFormat setDateFormat:@"yyyy"];
                    year= [dateFormat stringFromDate:[[_financialRatio2Array objectAtIndex:_financialRatioPostion2] uint16ToDate]];
                    [dateFormat setDateFormat:@"MM"];
                    month=[dateFormat stringFromDate:[[_financialRatio2Array objectAtIndex:_financialRatioPostion2] uint16ToDate]];
                }else{
                    [dateFormat setDateFormat:@"yyyy"];
                    year= [dateFormat stringFromDate:[[_financialRatio2Array lastObject] uint16ToDate]];
                    [dateFormat setDateFormat:@"MM"];
                    month=[dateFormat stringFromDate:[[_financialRatio2Array lastObject] uint16ToDate]];
                    _financialRatioPostion2 = (int)[_financialRatio2Array count]-1;
                }
                NSString * Q = [self dateToQuarter:month];
                if (item->identCode[0]=='T' && item->identCode[1]=='W') {
        ...
        titleLabel.text = NSLocalizedStringFromTable([_dataModal.financialRatio.keyArray objectAtIndex:section], @"Equity", nil);
...
-(void)readFromFile{
    ...
    NSString *path2 = [[CodingUtil fonestockDocumentsPath] stringByAppendingPathComponent:
                       [NSString stringWithFormat:@"FinanceMemory.plist"]];
...
-(void)discardData{
    ...
    [_financialRatio1Array removeAllObjects];
    [_financialRatio2Array removeAllObjects];
    _balanceGroup1Btn.titleLabel.text = @"";
    _balanceGroup2Btn.titleLabel.text = @"";
    _incomeGroup1Btn.titleLabel.text = @"";
    _incomeGroup2Btn.titleLabel.text = @"";
    _cashFlowGroup1Btn.titleLabel.text = @"";
    _cashFlowGroup2Btn.titleLabel.text = @"";
    _financialRatioGroup1Btn.titleLabel.text = @"";
    _financialRatioGroup2Btn.titleLabel.text = @"";
    [_dataModal.balanceSheet setTargetNotify:nil];
    [_dataModal.income setTargetNotify:nil];
    [_dataModal.cashFlow setTargetNotify:nil];
    [_dataModal.financialRatio setTargetNotify:nil];
